...
- *Claude Code IDE* -- full Claude Code CLI integration with MCP bridge
  ...
  Opens in a bottom side window. Typing in the Claude buffer automatically
  opens the minibuffer prompt (=send-prompt=), so you compose prompts from
  the minibuffer instead of the terminal. Slash commands (=/help=, etc.)
  still go directly to the terminal. Accessed via a *hydra* menu (=C-c c=).
...
*WakaTime* runs globally, silently logging coding activity. Only loads if
=wakatime-cli= is found on the system, so the config works cleanly on
machines without it.
...
- [[https://github.com/pdelfino/karabiner-config][karabiner-config]] - Emacs keybindings system-wide on macOS (Caps Lock → Control, etc.)
- [[https://github.com/pdelfino/claude-config][claude-config]] - Claude Code configuration (the AI leash)
